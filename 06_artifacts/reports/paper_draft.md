...
**主要贡献**：
1. **63 维可解释特征库**，含 5 个新族：近体诗格律（粘对/押韵/对仗）、段落主题 NLP、NER 意象顺序逻辑、真实声学信号（五度调值/元音开口度/韵母音位距离）、bge-small-zh 语义向量
2. **指标迭代闭环协议**：「发现失败模式 → 加反例训练 → 重训 → 验证」——8 个版本（v1→v8）的完整演进
...
5. **方法学警示（R17）**：单数据集 L2 ablation 的「删除特征更好」结论被多数据集重测证伪——特征选择必须跨切片验证

**关键数字**：
- **v4b（最终冻结指标）** 在固定验证集上 Quadratic Kappa = **0.974**，专家集 0.940，AI 诗假阳性 = 0
- 剔除标注噪声后，指标与干净人类多数票的 Kappa = **0.924**
- 真实人类 IAA（剔除噪声后）= **0.822**
- AI 仿诗评测集上 v2 假阳性 = 31，v4b 假阳性 = **0**
- 语义向量解决「明月↔霜 无共同字」：字符重叠 = 0 → bge 余弦 = **0.406**
- 17 轮实验探索：v4b 是收敛点，v6/v7/v8 均未突破（val 0.921-0.947 < 0.974）
...
我们的特征库经历了 v1→v8 的演进，最终包含 **12 个族、58 个自有特征**（+5 baseline = 63 维）。核心设计原则：
...
| **v4b** | + 32 AI 垃圾负样本 | **0.974** | **0.431** | **0.940** | **迭代闭环验证·最优** |
| v5 | + style 特征 | 0.957 | 0.438 | — | social/news 已 100% reject |
| v6a | − struct − style | 0.921 | — | 0.920 | **Racter 崩塌，废弃** |
| v6b | − struct − style + AI neg | 0.937 | 0.459* | **0.840** | **证伪 ablation，废弃** |
| v7b | + meter/theme/ner/semantic/phonetics | 0.947 | 0.438 | 0.900 | 可解释性提升 |
| v8b | + theme8 语义单元 + bge 实体 | 0.937 | — | 0.840 | 未超 v7b |

*R17 重测：v6b AI 诗 kappa=0.459（含 AI neg 训练，AI 集部分包含在训练中不纯）。

![版本演进](docs/figures/fig1_version_evolution.png)

> **⚠️ v6 重测警示（R17）**：L2 ablation 曾建议「drop struct/style 更好」
> （val +0.018/+0.017），但 v6a/v6b 在全评估上**均不如 v4b**——
> v6b 的 expert Kappa 掉到 **0.840**（v4b 0.940），Racter 崩塌再现。
> **ablation 结论只在 val 单集成立，未推广到多数据集**。最终保持 **v4b 冻结**。
...
- `struct` 和 `style` 在**单族单独评估**时表现较弱（0.920/0.911）
- `lang` / `music` 中性——可考虑简化

> **⚠️ 重要方法学警示（R17）**：ablation 显示「drop struct/style 使 val Kappa
> +0.018/+0.017」，但 **v6 重测证伪**了这一结论——drop 后 expert Kappa 从 0.94
> 崩塌到 0.84（Racter 样本全错）。**单数据集 ablation 的结论不可靠**，必须
> 在多数据集（val + expert + AI 集）上验证。这本身就是本研究的发现之一：
> 特征选择必须跨切片验证，否则会引入灾难性回归。

**L2 设计的科学意义**：直接回答方案 §1.3「人类判断诗歌时最依赖哪些特征」——**答案是「行数与古典格式」（form 族），与「文字意象密度」（lang 族）次之**。但「依赖哪些特征」≠「可以删除哪些特征」——struct 族的 `line_ending_punct` / `short_line_ratio` 虽单独评估弱，却是识别 Racter 这类「散文诗」的关键。
...
| 评估 | v1 | v2 | v3 | **v4b** | v5 | v6b | v7b | v8b |
|---|---|---|---|---|---|---|---|---|
| 验证集 Kappa | 0.983 | 0.940 | 0.940 | **0.974** | 0.957 | 0.937 | 0.947 | 0.937 |
| AI 诗 Kappa | — | -0.012 | -0.012 | **0.431** | 0.438 | 0.459 | 0.438 | — |
| 专家集 Kappa | — | 0.940 | — | **0.940** | — | 0.840 | 0.900 | 0.840 |
| 验证集 N | 120 | 371 | 371 | 371 | 371 | 371 | 371 | 371 |
| AI 诗 N | 0 | 209 | 209 | 209 | 209 | 209 | 209 | — |
| 主要变化 | 13 特征 | +lang | +purity | **+32 AI neg** | +style | −7 特征 | +5 新族 | +语义单元 |

**结论**：v4b 在所有版本中 val Kappa 最高（0.974），且 expert Kappa 保持 0.940 不退化。
v6/v7/v8 的尝试均未突破——**v4b 是结构性指标的收敛点**。
...
我们构建了中文诗歌「诗歌性」自动评测指标，并提出了**指标迭代闭环协议**。在 1855 条人工标注上达到 val Kappa 0.974（含 32 条 AI 仿诗反例），并通过 17 轮实验系统识别了 3 类失败模式（古典诗盲区、关键词匹配、AI 垃圾诗）。

**核心贡献**：
1. **63 维结构与语言学特征库**——含 5 个新族：近体诗格律、段落主题 NLP、NER 意象顺序逻辑、真实声学信号、bge 语义向量
2. **指标迭代闭环协议**——「发现失败 → 加反例 → 重训」，8 版本（v1→v8）演进
3. **首次基于真实生产标注的 IAA 评估**——揭示「真实 IAA = 0.50，含噪 30%」
4. **AI 仿诗的「指标失效」案例**——31 条夹带英文/乱码的 AI 诗，v4b 修复后假阳性归零
5. **语义向量解决意象盲区**——「明月↔霜」从字符重叠 0 到 bge 余弦 0.406
6. **方法学警示（R17）**——单数据集 ablation 结论被多数据集证伪，特征选择必须跨切片验证

**收敛结论**：v4b 是结构性指标的收敛点（val 0.974 / expert 0.940 / AI fp=0）。
v6（删特征）、v7（5 新族）、v8（语义单元）均未突破——**不是指标越复杂越好，
而是「发现失败 → 加反例」的闭环驱动了实质改善**（v2→v4b 的 kappa 从 0.94 提到 0.974）。

**M1-M6 路线图**：
- ✅ M1: v2 冻结 + 13 轮诊断
- ✅ M2: v4b 迭代（AI 垃圾反例）— **收敛点**
